...
def single_play(num, game, simulation):
    for i in range(args.game_num // args.parallel_num):
        ...
        while not player.board.is_game_over():
            ...
            if x != SIZE or y != SIZE:
                ...
                while current.is_expanded:
                    depth += 1
                    position = current.board.to_feature()
                    _, value = net.run(current.board)
                    aver += value
                    optimal = np.argmax(current.child_N)
                    current = current.maybe_add_child(optimal)
                    positions.append(position)
                
                for _ in range(depth):
                    avers.append(aver / depth)
        ...
        train_data = []
        for position, aver in zip(positions, avers):
            train_data.append((position, aver))
        train_data = np.array(train_data)
        now_time = datetime.datetime.now()
        ts = datetime.datetime.strftime(now_time, '%Y-%m-%d-%H%M%S')
        rd = random.randint(0, 100000000)
