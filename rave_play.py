...
parser.add_argument('--parallel_num', default=50, type=int)
args = parser.parse_args()

openings = random.sample([i for i in range(169)], args.game_num)
...
def single_play(net1, net2, game, simulation, old_winning, flag):
    ...
    for i in range(args.game_num // parallel_num):
        ...
        if game == 0:
            show = True
        print("Game:", game + i)
        player1 = MCTSPlayer(net=net1, simulations_per_move=simulation, th=0)
        player2 = RAVEPlayer(net=net2, simulations_per_move=simulation, th=0)
        player1.initialize_game()
        player2.initialize_game()
        player1.make_move(openings[game + i] // 13, openings[game + i] % 13)
        player2.make_move(openings[game + i] // 13, openings[game + i] % 13)
        color = hex.WHITE
...
def write_pgn(old_name, new_name, new_white, new_black):
    ...
    for _ in range(args.game_num):
        f.write("[Event \"F/S Return Match\"]\n")
        f.write("[Site \"Belgrade, Serbia JUG\"]\n")
        f.write("[Date \"1992.11.04\"]\n")
        f.write("[Round \"29\"]\n")
        f.write("[White \"" + white + "\"]\n")
        f.write("[Black \"" + black + "\"]\n")
        if whitewins > 0:
...
if __name__ == '__main__':
    ...
    pc1name = '20190315.model'
    pc2name = '20190315.model'

    nets1 = []
    nets2 = []
    for i in range(parallel):
        nets1.append(network.PV(pc1name, num=i%4))
        nets2.append(network.PV(pc2name, num=i%4))
    ...
    line = 'mcts vs rave' + ' = ' + str(old) + ':' + str(2 * game_num - old)
